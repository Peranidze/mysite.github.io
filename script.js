...
const block_site = document.querySelectorAll('.block_site');

block_site.forEach(el => {
    el.addEventListener('click', (e) => {
        const text_go = el.querySelector('.text_go');
        const btn_open = el.querySelector('.btn');
        el.classList.add('open_block_site')
        text_go.style.cssText = 'opacity: 0'
        if (e.target === btn_open) {
            el.classList.remove('open_block_site')
            text_go.style.cssText = 'opacity: 1'
        }
    })
})
